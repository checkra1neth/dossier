...
export function SwapReport({ data }: { data: SwapData }): ReactNode {
    ...
    <div ref={ref}>
      <div className="report-top">
        <div className="report-wallet">
          <h2>Swap: {inputToken.symbol} &rarr; {outputToken.symbol}</h2>
      ...
      <div className="stats-row">
        ...
        <div className="stat">
          ...
          <div className="stat-value">{inputToken.chain}</div>
        </div>
      </div>
        ...
        <div>
          ...
          <div className="token-info">
            <div className="token-detail">
              <strong>{inputToken.name} ({inputToken.symbol})</strong>
              Price: {fmtUsd(inputToken.price)} &middot; Chain: {inputToken.chain}
            </div>
            <div className="token-detail">
              <strong>{outputToken.name} ({outputToken.symbol})</strong>
              Price: {fmtUsd(outputToken.price)} &middot; Chain: {outputToken.chain}
            </div>
          </div>
          {data.txHash && (
